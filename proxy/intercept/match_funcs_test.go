...
var (
	matchNoRequests  MatchRequestFunc  = func(r *ProxyRequest) (bool, error) { return false, nil }
	matchNoResponses MatchResponseFunc = func(r *ProxyRequest, rs *ProxyResponse) (bool, error) { return false, nil }
)

func TestMatchRequestURL(t *testing.T) {
	u, _ := url.Parse("http://some.domain.com:8080/somepath/?someqs=someval")
	rq := ProxyRequest{URL: *u}
...
func TestMatchResponseCode(t *testing.T) {
	rs := ProxyResponse{Status: "200 OK"}
	...
	test := func(t *testing.T, pattern string, matchRequest bool, expected bool) {
		...
		matchFunc := MatchResponseStatus(pattern, matchRqFunc)

		match, err := matchFunc(&ProxyRequest{}, &rs)
